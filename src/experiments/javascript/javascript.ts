...
function writeCode(content: VElement[]) {
    ...
    const newVElement = experimentSyntaxJavaScriptComponent();
...
async function loadFile(filePath: string, storeFunction: (content: VElement[]) => void) {
    try {
        ...
        storeFunction(highlight(content, JavaScriptParser));
    } catch (error) {
        console.error('Error loading file:', error);
    }
}

function experimentSyntaxJavaScriptComponent(): VElement {
...
function experimentSyntaxJavaScriptPage(): VNode {
    return h('div', {},
        pageHeader(),
        h('main', {className: 'main'},
            h('article', {},
                h('h1', {}, 'JavaScript syntax example'),
                experimentSyntaxJavaScriptComponent()
            ),
        ),
        pageFooter()
    );
...
export const experimentSyntaxJavaScript = new ExperimentPage(
    '/experiments/javascript',
    experimentSyntaxJavaScriptPage
);
