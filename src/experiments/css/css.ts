...
import {CSSParser} from '../../lib/syntax/CSSParser';
...
function writeCode(content: VElement[]) {
    ...
    const newVElement = experimentSyntaxCSSComponent();
...
async function loadFile(filePath: string, storeFunction: (content: VElement[]) => void) {
    try {
        ...
        storeFunction(highlight(content, CSSParser));
    } catch (error) {
        console.error('Error loading file:', error);
    }
}

function experimentSyntaxCSSComponent(): VElement {
    ...
    contents.mountCallback = () => {
        ...
        if (code.length === 0) {
            loadFile('/experiments/css/test.css', writeCode);
        }
    }
...
function experimentSyntaxCSSPage(): VNode {
    return h('div', {},
        pageHeader(),
        h('main', {className: 'main'},
            h('article', {},
                h('h1', {}, 'CSS syntax example'),
                experimentSyntaxCSSComponent()
            ),
        ),
        pageFooter()
    );
...
export const experimentSyntaxCSS = new ExperimentPage(
    '/experiments/css',
    experimentSyntaxCSSPage
);
